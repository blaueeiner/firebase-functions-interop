...
void main() {
  ...
  group('$HttpsFunctions', () {
    ...
    test('save to database', () async {
      ...
      var response = await http.get(Uri.parse('$baseUrl/httpsToDatabase?name=Firebase$time'));
      expect(response.statusCode, 200);
      expect(response.body, 'httpsToDatabase: ok\n');

      var snapshot =
          await app.database().ref('/tests/httpsToDatabase/original').once<String>('value');
      expect(snapshot.val(), 'FIREBASE$time');
    });

    test('get json body', () async {
      var response = await http.post(Uri.parse('$baseUrl/jsonTest'),
          headers: {'Content-Type': 'application/json'}, body: json.encode({"helloJSON": "hi"}));
      expect(response.statusCode, 200);
